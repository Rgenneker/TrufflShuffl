...
const INVESTMENT_TYPES = [
  "Savings Account", "Fixed Deposit", "Money Market", "Unit Trust",
  "Stocks & Shares", "Retirement Annuity", "Tax-Free Savings Account",
];
...
export default function InvestmentCalculator() {
  ...
  function calculate() {
    const r = calculateInvestment(
      Number(principal), Number(monthly), Number(rate), Number(years), Number(freq)
    );
    setResult(r);
  }
  ...
  return (
    <CalculatorLayout
      ...
      description="Compare compound vs simple interest growth and project how your money grows over time with lump sums, regular contributions, or both."
      instructions={
        <ol className="list-decimal list-inside space-y-2 text-sm text-muted-foreground">
          ...
          <li>Set the investment term in years</li>
          <li>Click Calculate to see compound vs simple growth side by side</li>
        </ol>
      }
          ...
          <div className="space-y-6 pt-2">
            <div className="grid grid-cols-2 gap-3">
              <div className="bg-background rounded-lg p-4 border border-primary/30">
                ...
                <p className="font-mono text-xl font-bold text-primary" data-testid="result-compound-final">R {fmt(result.finalValueCompound)}</p>
                <p className="text-xs text-green-400 mt-1">Interest: R {fmt(result.totalInterestCompound)}</p>
              </div>
              <div className="bg-background rounded-lg p-4 border border-border">
                <p className="text-xs text-muted-foreground mb-1">Simple Interest Final Value</p>
                <p className="font-mono text-xl font-bold text-foreground" data-testid="result-simple-final">R {fmt(result.finalValueSimple)}</p>
                <p className="text-xs text-muted-foreground mt-1">Interest: R {fmt(result.totalInterestSimple)}</p>
              </div>
            </div>
            ...
            <div className="bg-background rounded-lg p-4 border border-border">
              ...
              <p className="font-mono text-2xl font-bold text-foreground">
                R {fmt(result.finalValueCompound - result.finalValueSimple)}
              </p>
            ...
            <table className="w-full text-xs border-collapse">
              <thead>
                <tr className="text-muted-foreground">
                  ...
                  <th className="text-right py-2 border-b border-border">Compound (R)</th>
                  <th className="text-right py-2 border-b border-border">Simple (R)</th>
                  <th className="text-right py-2 border-b border-border">Contributions (R)</th>
                </tr>
              </thead>
              <tbody>
                {result.data.map(row => (
